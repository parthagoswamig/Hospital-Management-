...
export class InitialSchema1760671827278 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."users_role_enum" AS ENUM('SUPER_ADMIN', 'TENANT_ADMIN', 'HOSPITAL_ADMIN', 'DOCTOR', 'NURSE', 'LAB_TECHNICIAN', 'RADIOLOGIST', 'PHARMACIST', 'RECEPTIONIST', 'ACCOUNTANT', 'PATIENT', 'USER')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."tenants_type_enum" AS ENUM('hospital', 'clinic', 'diagnostic_center', 'pharmacy', 'laboratory')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."tenants_status_enum" AS ENUM('active', 'inactive', 'suspended', 'trial', 'pending')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."tenants_subscription_plan_enum" AS ENUM('free', 'basic', 'professional', 'enterprise')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."audit_logs_action_enum" AS ENUM('create', 'read', 'update', 'delete', 'login', 'logout', 'login_failed', 'password_change', 'password_reset', 'export', 'print', 'share', 'approve', 'reject', 'sign')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."audit_logs_entity_type_enum" AS ENUM('user', 'patient', 'appointment', 'medical_record', 'prescription', 'lab_order', 'lab_result', 'billing', 'invoice', 'payment', 'inventory', 'medication', 'tenant', 'setting')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TYPE "public"."tenants_subscription_plan_enum"`,
    );
